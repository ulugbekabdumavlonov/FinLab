    ...
    <div className="mx-4 mb-4 relative" ref={ref}>
      ...
      >
        ...
        <div className="flex-1 text-left overflow-hidden">
          <p className="text-[10px] text-white/40 leading-none mb-0.5">Вы вошли как</p>
          ...
          >
            <div className="flex items-center gap-3 px-4 py-3 border-b border-white/[0.07]">
              ...
              <div className="overflow-hidden flex-1">
                ...
                <p className="text-[11px] text-white/40 truncate leading-tight mt-0.5">{email}</p>
...
export default function Sidebar() {
  ...
  // владелец видит всё, сотрудник — только разрешённые пункты
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-5 flex items-center justify-between">
        ...
        <button onClick={() => setMobileOpen(false)} className="md:hidden text-white/50 hover:text-white transition">
          <X size={22} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 space-y-4">
        ...
        <Section title="Отчёты" open={openReports} toggle={() => setOpenReports(!openReports)}>
          {can("dashboard") && <NavItem to="/app" icon={<LayoutDashboard size={18} />} label="Dashboard" active={isActive("/app")} />}
          {can("dds")       && <NavItem to="/app/cashflow" icon={<Wallet size={18} />} label="ДДС" active={isActive("/app/cashflow")} />}
          {can("pl")        && <NavItem to="/app/pl" icon={<BarChart3 size={18} />} label="P&L" active={isActive("/app/pl")} />}
          {can("balance")   && <NavItem to="/app/balance" icon={<PieChart size={18} />} label="Баланс" active={isActive("/app/balance")} />}
        </Section>

        <Section title="Зарплата и Сотрудники" open={openPayroll} toggle={() => setOpenPayroll(!openPayroll)}>
          {can("employees") && <NavItem to="/app/Employees" icon={<Users size={18} />} label="Сотрудники" active={isActive("/app/Employees")} />}
          {can("payroll")   && <NavItem to="/app/Salarystatement" icon={<FileUser size={18} />} label="Ведомость" active={isActive("/app/Salarystatement")} />}
        </Section>

        <Section title="Система" open={openSystem} toggle={() => setOpenSystem(!openSystem)}>
          {can("settings")   && <NavItem to="/app/settings" icon={<Settings size={18} />} label="Настройки" active={isActive("/app/settings")} />}
          {can("operations") && <NavItem to="/app/Operations" icon={<ArrowLeftRight size={18} />} label="Операции" active={isActive("/app/Operations")} />}
          {can("import")     && <NavItem to="/app/Database" icon={<CloudUpload size={18} />} label="Импорт" active={isActive("/app/Database")} />}
        </Section>

        <Section title="Справочники" open={openInfo} toggle={() => setOpenInfo(!openInfo)}>
          {can("accounts")      && <NavItem to="/app/MyWallet" icon={<FileText size={18} />} label="Мои счета" active={isActive("/app/MyWallet")} />}
          {can("legalEntities") && <NavItem to="/app/MyCompany" icon={<LayoutList size={18} />} label="Мои юрлица" active={isActive("/app/MyCompany")} />}
          {can("categories")    && <NavItem to="/app/MyCategories" icon={<Folder size={18} />} label="Мои статьи" active={isActive("/app/MyCategories")} />}
          {can("projects")      && <NavItem to="/app/MyProjects" icon={<Folder size={18} />} label="Мои проекты" active={isActive("/app/MyProjects")} />}
          {isOwner              && <NavItem to="/app/AccrualsPage" icon={<Folder size={18} />} label="Начисления" active={isActive("/app/AccrualsPage")} />}
          {isOwner              && <NavItem to="/app/Counterpartiespage" icon={<Folder size={18} />} label="Контрагенты" active={isActive("/app/Counterpartiespage")} />}
          {isOwner              && <NavItem to="/app/Settlements" icon={<Folder size={18} />} label="Взаиморасчеты" active={isActive("/app/Settlements")} />}
        </Section>
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 flex items-center justify-between
        ...
        <button onClick={() => setMobileOpen(true)} className="text-white/70 hover:text-white transition p-1">
          <Menu size={24} />
        </button>
      </div>
