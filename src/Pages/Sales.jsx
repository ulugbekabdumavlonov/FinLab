...
const DOC_TYPES = [
  ...
  { id: "shipment", label: "Накладная",   icon: "📦", color: "#185FA5", bg: "#E6F1FB" },
...
const DOC_STATUSES = [
  ...
  { id: "shipped",   label: "Отгружен",    color: "#0F6E56", bg: "#E1F5EE" },
...
function calcTotals(items = []) {
  let subtotal = 0, vatAmount = 0, costTotal = 0;
  items.forEach((item) => {
  ...
  return { subtotal, vatAmount, total: subtotal + vatAmount, costTotal };
}

// ─── CSV Export ────────────────────────────────────────────────────────────────
function exportCSV(docs) {
  const h = ["Дата","Номер","Тип","Клиент","Статус","Позиций","Сумма б/НДС","НДС","Итого","Оплачено","Долг","Срок оплаты"];
  ...
  a.href = url; a.download = `продажи_${todayISO()}.csv`; a.click();
...
// ─── Animated number ───────────────────────────────────────────────────────────
function AnimNum({ value, color, large }) {
  ...
  return <span style={{ color, fontSize: large ? 22 : undefined, fontWeight: large ? 700 : undefined }}>{fmtK(disp)}</span>;
}
...
function TypeBadge({ typeId, small }) {
  ...
  return (
    <span style={{ fontSize: small ? 11 : 12, padding: small ? "2px 7px" : "3px 10px", borderRadius: 20, fontWeight: 500, background: t.bg, color: t.color, display: "inline-flex", alignItems: "center", gap: 4, whiteSpace: "nowrap", border: `0.5px solid ${t.color}22` }}>
...
function StatusBadge({ statusId, small }) {
  ...
  return (
    <span style={{ fontSize: small ? 11 : 12, padding: small ? "2px 7px" : "3px 10px", borderRadius: 20, fontWeight: 500, background: s.bg, color: s.color, whiteSpace: "nowrap", border: `0.5px solid ${s.color}22` }}>
      {s.label}
    </span>
  );
}
...
// ─── Product inline search (for document lines) ────────────────────────────────
function ProductSearch({ products, lineId, name, type, onSelect, onChangeName, onChangeType }) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "flex", gap: 4 }}>
      <button onClick={() => onChangeType(type === "product" ? "service" : "product")}
        title={type === "product" ? "Товар → нажми для смены на Услугу" : "Услуга → нажми для смены на Товар"}
        style={{ flexShrink: 0, fontSize: 13, background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-secondary)", borderRadius: "var(--border-radius-md)", cursor: "pointer", padding: "4px 7px", lineHeight: 1 }}>
        {type === "service" ? "🛠" : "📦"}
      </button>
      <input value={q}
      ...
      {open && filtered.length > 0 && (
        <div style={{ position: "absolute", top: "calc(100% + 2px)", left: 32, right: 0, zIndex: 700, background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-secondary)", borderRadius: "var(--border-radius-md)", maxHeight: 200, overflowY: "auto", boxShadow: "0 4px 16px rgba(0,0,0,0.12)" }}>
          {filtered.map((p) => (
            <div key={p.id} onClick={() => { onSelect(p); setQ(p.name); setOpen(false); }}
              ...
              <span style={{ fontSize: 12, color: "#185FA5", fontWeight: 600, marginLeft: 8, flexShrink: 0 }}>{fmt(p.salePrice || 0)}</span>
            </div>
          ))}
        </div>
      )}
...
function LinesEditor({ lines, onChange, products }) {
  const add = () => onChange([...lines, {
    ...
    qty: 1, price: "", costPrice: 0, discount: 0, vat: 0,
  }]);
  ...
  const pick = (lineId, p) => {
    onChange(lines.map((l) => l.id === lineId ? {
      ...
      unit: p.unit || "шт", price: p.salePrice || 0,
      costPrice: p.costPrice || 0, vat: p.vat || 0,
    } : l));
  };
  ...
  return (
    <div>
      {/* Column headers */}
      <div style={{ display: "grid", gridTemplateColumns: "24px 1fr 64px 76px 80px 64px 60px 80px 24px", gap: 4, marginBottom: 6, padding: "0 0 6px", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
        {["#", "Наименование", "Ед", "Кол-во", "Цена", "Скид%", "НДС%", "Сумма", ""].map((h, i) => (
          <div key={i} style={{ fontSize: 10, fontWeight: 500, color: "var(--color-text-tertiary)", textTransform: "uppercase", letterSpacing: "0.04em", textAlign: i >= 3 && i <= 7 ? "right" : "left" }}>{h}</div>
        ))}
      </div>

      {/* Lines */}
      {lines.map((l, idx) => (
        <div key={l.id} style={{ display: "grid", gridTemplateColumns: "24px 1fr 64px 76px 80px 64px 60px 80px 24px", gap: 4, marginBottom: 4, alignItems: "center" }}>
          <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", textAlign: "center" }}>{idx + 1}</div>
          <ProductSearch
            products={products} lineId={l.id}
          ...
          <select value={l.vat} onChange={(e) => set(l.id, "vat", Number(e.target.value))}
            style={{ ...inp, cursor: "pointer" }}>
            {VAT_RATES.map((r) => <option key={r} value={r}>{r}%</option>)}
          </select>
          <div style={{ fontSize: 13, fontWeight: 500, textAlign: "right", color: "var(--color-text-primary)", whiteSpace: "nowrap" }}>{fmt(lineTotal(l))}</div>
      ...
      <button onClick={add} style={{ marginTop: 8, padding: "6px 14px", border: "0.5px dashed var(--color-border-secondary)", borderRadius: "var(--border-radius-md)", background: "none", fontSize: 13, cursor: "pointer", color: "var(--color-text-secondary)", display: "flex", alignItems: "center", gap: 6 }}>
      ...
      {lines.length > 0 && (
        <div style={{ marginTop: 12, borderTop: "0.5px solid var(--color-border-tertiary)", paddingTop: 10 }}>
          {[
            ...
            ["Себестоимость", fmt(totals.costTotal), "var(--color-text-tertiary)"],
            ["Маржа",
              totals.subtotal > 0
                ? `${fmt(totals.subtotal - totals.costTotal)} (${Math.round((totals.subtotal - totals.costTotal) / totals.subtotal * 100)}%)`
                : "—",
              totals.subtotal > totals.costTotal ? "#3B6D11" : "#A32D2D"],
            ["ИТОГО К ОПЛАТЕ", fmt(totals.total), "#185FA5"],
          ].map(([label, val, color]) => (
            <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
              ...
              <span style={{ fontSize: label === "ИТОГО К ОПЛАТЕ" ? 16 : 13, fontWeight: label === "ИТОГО К ОПЛАТЕ" ? 700 : 500, color }}>{val}</span>
            </div>
          ))}
        </div>
      )}
...
function DocModal({ docData, products, counterparties, accounts, projects, onSave, onDelete, onClose }) {
  ...
  const makeNumber = () => `${new Date().getFullYear()}-${String(Date.now()).slice(-5)}`;

  const [form, setForm] = useState(() =>
    docData
      ? { ...docData }
      : { _isoDate: todayISO(), dueDate: "", number: makeNumber(), type: "invoice", status: "draft", clientId: "", clientName: "", walletId: "", walletName: "", project: "", notes: "", items: [], paid: 0 }
  );
  ...
  const handleSave = async () => {
    if (saving || !form.clientName) return;
    setSaving(true);
    try {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 500 }}>
      <div style={{ background: "var(--color-background-primary)", borderRadius: "var(--border-radius-lg)", border: "0.5px solid var(--color-border-secondary)", width: 740, maxWidth: "97vw", maxHeight: "94vh", display: "flex", flexDirection: "column", boxShadow: "0 16px 48px rgba(0,0,0,0.18)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
            <div>
              <h2 style={{ margin: 0, fontSize: 17, fontWeight: 500 }}>
                {isEdit ? `${form.number}` : "Новый документ продажи"}
              </h2>
              {isEdit && (
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
            {DOC_TYPES.map((t) => (
              <button key={t.id} onClick={() => set("type", t.id)}
                style={{ flex: 1, padding: "7px 4px", borderRadius: "var(--border-radius-md)", border: "0.5px solid var(--color-border-secondary)", cursor: "pointer", fontSize: 12, fontWeight: form.type === t.id ? 500 : 400, background: form.type === t.id ? t.bg : "var(--color-background-secondary)", color: form.type === t.id ? t.color : "var(--color-text-secondary)", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
          ...
          <div style={{ display: "flex", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
            {TABS.map(([key, label]) => (
              <div key={key} onClick={() => setTab(key)}
                style={{ padding: "8px 16px", fontSize: 14, cursor: "pointer", borderBottom: tab === key ? "2px solid #3b62d6" : "2px solid transparent", color: tab === key ? "#3b62d6" : "var(--color-text-secondary)", fontWeight: tab === key ? 500 : 400 }}>
                {label}
              </div>
            ))}
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px" }}>
          ...
          {tab === "main" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <div style={lbl}>Клиент / Покупатель *</div>
                  <Dropdown
                    options={counterparties}
                    value={form.clientName}
                    onChange={(v) => set("clientName", v)}
                    placeholder="Выбрать клиента" zIndex={600}
                  />
                </div>
                <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <div style={lbl}>Счёт поступления</div>
                  <Dropdown
                    options={accounts}
                    value={form.walletId}
                    onChange={(v, obj) => { set("walletId", v); set("walletName", obj?.name || ""); }}
                    placeholder="Выбрать счёт" zIndex={600}
                    getLabel={(w) => w.name}
                    getValue={(w) => w.id}
                  />
                </div>
                <div>
                  ...
                  <Dropdown
                    ...
                    placeholder="Выбрать проект" zIndex={600}
                    getLabel={(p) => p.name}
                    getValue={(p) => p.name}
                  />
                </div>
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={(e) => set("notes", e.target.value)}
                  placeholder="Договор №, условия, комментарий…"
                  style={{ ...inp, minHeight: 72, resize: "vertical", lineHeight: 1.5 }} />
              </div>

              {/* Quick totals if items exist */}
              {form.items.length > 0 && (
                <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "12px 16px", border: "0.5px solid var(--color-border-tertiary)", display: "flex", gap: 24 }}>
                  {[["Без НДС", fmt(totals.subtotal), "var(--color-text-primary)"], ["НДС", fmt(totals.vatAmount), "var(--color-text-secondary)"], ["Итого", fmt(totals.total), "#185FA5"]].map(([l, v, c]) => (
                    <div key={l}>
                  ...
                  {totals.costTotal > 0 && (
                    <div style={{ marginLeft: "auto" }}>
                      <div style={{ fontSize: 11, color: "var(--color-text-tertiary)" }}>Маржа</div>
                      <div style={{ fontSize: 15, fontWeight: 600, color: totals.subtotal > totals.costTotal ? "#3B6D11" : "#A32D2D" }}>
                        {Math.round((totals.subtotal - totals.costTotal) / (totals.subtotal || 1) * 100)}%
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                <div>
                  ...
                  <div style={{ padding: "10px 14px", border: "0.5px solid var(--color-border-secondary)", borderRadius: "var(--border-radius-md)", background: "var(--color-background-secondary)", fontSize: 18, fontWeight: 700, color: "#185FA5" }}>{fmt(totals.total)}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div style={{ padding: "10px 14px", borderRadius: "var(--border-radius-md)", border: `0.5px solid ${isPaidFull ? "#3B6D1133" : "#A32D2D33"}`, background: isPaidFull ? "#EAF3DE" : "#FCEBEB", fontSize: 16, fontWeight: 700, color: isPaidFull ? "#3B6D11" : "#A32D2D" }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {DOC_STATUSES.map((s) => (
                    <button key={s.id} onClick={() => set("status", s.id)}
                      style={{ padding: "8px 14px", borderRadius: "var(--border-radius-md)", border: form.status === s.id ? `1.5px solid ${s.color}` : "0.5px solid var(--color-border-secondary)", background: form.status === s.id ? s.bg : "var(--color-background-secondary)", color: form.status === s.id ? s.color : "var(--color-text-secondary)", fontSize: 13, fontWeight: form.status === s.id ? 600 : 400, cursor: "pointer" }}>
                      {s.label}
                    </button>
                  ))}
                </div>
              ...
              {/* Auto-status helper */}
              <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "10px 14px", fontSize: 12, color: "var(--color-text-tertiary)", border: "0.5px solid var(--color-border-tertiary)" }}>
                💡 Подсказка: при полной оплате переводите статус в «Оплачен». Если частично — «Частично». Если срок прошёл — «Просрочен».
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ padding: "14px 24px 20px", borderTop: "0.5px solid var(--color-border-tertiary)", flexShrink: 0, display: "flex", gap: 10 }}>
          <button onClick={handleSave} disabled={saving || !form.clientName}
            style={{ flex: 1, padding: "10px 0", background: (saving || !form.clientName) ? "#8ea4ea" : "#3b62d6", color: "#fff", border: "none", borderRadius: "var(--border-radius-md)", fontSize: 14, fontWeight: 500, cursor: (saving || !form.clientName) ? "not-allowed" : "pointer", opacity: (saving || !form.clientName) ? 0.75 : 1 }}>
...
// FILTER MODAL
// ═══════════════════════════════════════════════════════════════════════════════
const EMPTY_FILTERS = { type: "all", status: "all", client: "", dateFrom: "", dateTo: "", dueDateFrom: "", dueDateTo: "", amountFrom: "", amountTo: "", hasDebt: false };
...
function FilterModal({ filters, counterparties, onApply, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 500 }}>
      <div style={{ background: "var(--color-background-primary)", borderRadius: "var(--border-radius-lg)", border: "0.5px solid var(--color-border-secondary)", width: 500, maxWidth: "95vw", maxHeight: "92vh", overflowY: "auto", padding: "24px 28px 20px" }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {[{ id: "all", label: "Все", icon: "" }, ...DOC_TYPES].map((t) => (
              <button key={t.id} onClick={() => set("type", t.id)}
                style={{ padding: "5px 12px", borderRadius: 20, border: "0.5px solid var(--color-border-secondary)", fontSize: 12, cursor: "pointer", fontWeight: f.type === t.id ? 500 : 400, background: f.type === t.id ? (TYPE_MAP[t.id]?.bg || "var(--color-background-info)") : "var(--color-background-secondary)", color: f.type === t.id ? (TYPE_MAP[t.id]?.color || "var(--color-text-info)") : "var(--color-text-secondary)" }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {[{ id: "all", label: "Все" }, ...DOC_STATUSES].map((s) => (
              <button key={s.id} onClick={() => set("status", s.id)}
                style={{ padding: "5px 12px", borderRadius: 20, border: "0.5px solid var(--color-border-secondary)", fontSize: 12, cursor: "pointer", fontWeight: f.status === s.id ? 500 : 400, background: f.status === s.id ? (STATUS_MAP[s.id]?.bg || "var(--color-background-info)") : "var(--color-background-secondary)", color: f.status === s.id ? (STATUS_MAP[s.id]?.color || "var(--color-text-info)") : "var(--color-text-secondary)" }}>
                {s.label}
              </button>
            ))}
          </div>
        ...
        {/* Client */}
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 13, color: "var(--color-text-secondary)", marginBottom: 4 }}>Клиент</div>
          <Dropdown options={counterparties} value={f.client} onChange={(v) => set("client", v)} placeholder="Все клиенты" />
        </div>
        ...
        <div style={{ marginBottom: 14, display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <label htmlFor="hasDebt" style={{ fontSize: 13, color: "var(--color-text-primary)", cursor: "pointer" }}>Только с долгом</label>
        </div>
...
// ДЕБИТОРКА TAB
// ═══════════════════════════════════════════════════════════════════════════════
function DebtTab({ documents, onOpenDoc }) {
  const debtors = useMemo(() => {
    ...
    documents.forEach((d) => {
      ...
      if (debt < 0.01) return;
      const c = d.clientName || "Без клиента";
      if (!map[c]) map[c] = { client: c, docs: [], totalDebt: 0, overdueDebt: 0 };
      const isOverdue = d.dueDate && d.dueDate < todayISO();
      map[c].docs.push({ ...d, debt, isOverdue });
      map[c].totalDebt    += debt;
      map[c].overdueDebt  += isOverdue ? debt : 0;
    });
    return Object.values(map).sort((a, b) => b.totalDebt - a.totalDebt);
  }, [documents]);

  const totalDebt    = debtors.reduce((s, d) => s + d.totalDebt, 0);
  const overdueDebt  = debtors.reduce((s, d) => s + d.overdueDebt, 0);
  const overdueCount = debtors.reduce((s, d) => s + d.docs.filter((x) => x.isOverdue).length, 0);
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
        {[
          { label: "Общий долг", value: totalDebt, color: "#185FA5", large: true },
          { label: "Просрочено", value: overdueDebt, color: "#A32D2D", large: true },
          { label: "Клиентов-должников", value: debtors.length, color: "var(--color-text-primary)", plain: debtors.length },
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {debtors.map((debtor) => {
          const isOpen = expanded.has(debtor.client);
          const [avatarBg, avatarColor] = avatarStyle(debtor.client);
          return (
            <div key={debtor.client} style={{ background: "var(--color-background-primary)", borderRadius: "var(--border-radius-lg)", border: debtor.overdueDebt > 0 ? "0.5px solid #A32D2D44" : "0.5px solid var(--color-border-secondary)", overflow: "hidden" }}>
              {/* Card header */}
              <div onClick={() => toggle(debtor.client)}
                style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", cursor: "pointer" }}
                onMouseEnter={(e) => e.currentTarget.style.background = "var(--color-background-secondary)"}
                onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: avatarBg, color: avatarColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{initials(debtor.client)}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>{debtor.client}</div>
                  <div style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginTop: 2 }}>
                    {debtor.docs.length} документ{debtor.docs.length > 4 ? "ов" : debtor.docs.length > 1 ? "а" : ""}
                    {debtor.overdueDebt > 0 && <span style={{ marginLeft: 8, color: "#A32D2D", fontWeight: 600 }}>⚠ просрочено {fmt(debtor.overdueDebt)}</span>}
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 18, fontWeight: 700, color: debtor.overdueDebt > 0 ? "#A32D2D" : "#185FA5" }}>{fmt(debtor.totalDebt)}</div>
                  <div style={{ fontSize: 11, color: "var(--color-text-tertiary)" }}>долг</div>
                </div>
              ...
              {isOpen && (
                <div style={{ borderTop: "0.5px solid var(--color-border-tertiary)" }}>
                  {debtor.docs.map((d) => {
                    ...
                    return (
                      <div key={d.id} onClick={() => onOpenDoc(d)}
                        style={{ display: "grid", gridTemplateColumns: "1fr 110px 100px 100px 90px 90px", padding: "10px 18px 10px 70px", borderBottom: "0.5px solid var(--color-border-tertiary)", cursor: "pointer", alignItems: "center" }}
                        onMouseEnter={(e) => e.currentTarget.style.background = "var(--color-background-secondary)"}
                        onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                        <div>
                        ...
                        <div style={{ textAlign: "right" }}>
                          <div style={{ fontSize: 13, fontWeight: 700, color: d.isOverdue ? "#A32D2D" : "#185FA5" }}>{fmt(d.debt)}</div>
                          {dl !== null && (
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 110px 100px 100px 90px 90px", padding: "8px 18px 8px 70px", background: "var(--color-background-secondary)", fontSize: 12, fontWeight: 600, color: "var(--color-text-secondary)", alignItems: "center" }}>
                    <div>Итого по клиенту</div>
                    <div /><div />
                    <div style={{ textAlign: "right" }}>{fmt(debtor.docs.reduce((s, d) => s + (d.total || 0), 0))}</div>
                    <div style={{ textAlign: "right" }}>{fmt(debtor.docs.reduce((s, d) => s + (d.paid || 0), 0))}</div>
                    <div style={{ textAlign: "right", color: "#A32D2D" }}>{fmt(debtor.totalDebt)}</div>
                  </div>
                </div>
              )}
            </div>
      ...
      {debtors.length > 0 && (
        <div style={{ marginTop: 24, background: "var(--color-background-primary)", borderRadius: "var(--border-radius-lg)", border: "0.5px solid var(--color-border-secondary)", padding: "18px" }}>
          <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 14 }}>Анализ задолженности по срокам (aging)</div>
          {(() => {
            const buckets = [
            ...
            const allDebtDocs = debtors.flatMap((d) => d.docs);
...
function AnalyticsTab({ documents }) {
  ...
  const stats = useMemo(() => {
    ...
    const revenue    = paid.reduce((s, d) => s + (d.subtotal || 0), 0);
    const vatTotal   = paid.reduce((s, d) => s + (d.vatAmount || 0), 0);
    const grossTotal = paid.reduce((s, d) => s + (d.total || 0), 0);
    const costTotal  = paid.reduce((s, d) => s + (d.costTotal || 0), 0);
    const margin     = revenue > 0 ? ((revenue - costTotal) / revenue * 100) : 0;
    const returnAmt  = returns.reduce((s, d) => s + (d.total || 0), 0);

    // Monthly (last N months depending on period)
    ...
    for (let i = monthCount - 1; i >= 0; i--) {
      ...
      months.push({
        ...
        revenue: mDocs.reduce((s, doc) => s + (doc.total || 0), 0),
        count: mDocs.length,
        margin: mDocs.reduce((s, doc) => s + ((doc.subtotal || 0) - (doc.costTotal || 0)), 0),
      });
    }
    ...
    docs.forEach((d) => {
      ...
      if (!cMap[c]) cMap[c] = { name: c, count: 0, total: 0, paid: 0, margin: 0 };
      cMap[c].count++;
      cMap[c].total += d.total || 0;
      if (d.status === "paid") { cMap[c].paid += d.total || 0; cMap[c].margin += (d.subtotal || 0) - (d.costTotal || 0); }
    });
    const topClients = Object.values(cMap).sort((a, b) => b.total - a.total).slice(0, 8);
    ...
    docs.forEach((d) => {
      (d.items || []).forEach((item) => {
        const key = item.name || "—";
        if (!pMap[key]) pMap[key] = { name: key, type: item.type || "product", qty: 0, revenue: 0, margin: 0 };
        const base = (parseFloat(item.price) || 0) * (parseFloat(item.qty) || 0) * (1 - (parseFloat(item.discount) || 0) / 100);
        const cost = (parseFloat(item.costPrice) || 0) * (parseFloat(item.qty) || 0);
        pMap[key].qty     += parseFloat(item.qty) || 0;
        pMap[key].revenue += base;
        pMap[key].margin  += base - cost;
      });
    });
    const topProducts = Object.values(pMap).sort((a, b) => b.revenue - a.revenue).slice(0, 8);

    return { revenue, vatTotal, grossTotal, costTotal, margin, returnAmt, months, byType, topClients, topProducts };
  }, [docs, period]);

  const maxMonthRev = Math.max(...stats.months.map((m) => m.revenue), 1);
  const maxProd     = Math.max(...stats.topProducts.map((p) => p.revenue), 1);
  const maxClient   = Math.max(...stats.topClients.map((c) => c.total), 1);
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 20 }}>
        ...
        {[["1m","1 мес."],["3m","3 мес."],["6m","6 мес."],["1y","1 год"],["all","Всё время"]].map(([key, label]) => (
          <button key={key} onClick={() => setPeriod(key)}
            style={{ padding: "5px 14px", borderRadius: 20, border: "0.5px solid var(--color-border-secondary)", fontSize: 13, cursor: "pointer", background: period === key ? "var(--color-background-info)" : "var(--color-background-secondary)", color: period === key ? "var(--color-text-info)" : "var(--color-text-secondary)", fontWeight: period === key ? 500 : 400 }}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10, marginBottom: 20 }}>
        {[
          { label: "Выручка б/НДС",    value: stats.revenue,    color: "#3B6D11" },
          { label: "НДС",              value: stats.vatTotal,   color: "#185FA5" },
          { label: "Итого с НДС",      value: stats.grossTotal, color: "#185FA5" },
          { label: "Маржа",
            value: `${fmt(stats.revenue - stats.costTotal)} (${Math.round(stats.margin)}%)`,
            color: stats.margin >= 20 ? "#3B6D11" : stats.margin >= 0 ? "#854F0B" : "#A32D2D",
            plain: true },
          { label: "Возвраты",         value: stats.returnAmt,  color: "#A32D2D" },
          ...
          <div key={label} style={{ background: "var(--color-background-primary)", borderRadius: "var(--border-radius-md)", border: "0.5px solid var(--color-border-secondary)", padding: "14px 16px" }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color }}>
              {plain ? value : <AnimNum value={value} color={color} />}
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        ...
        <div style={{ background: "var(--color-background-primary)", borderRadius: "var(--border-radius-lg)", border: "0.5px solid var(--color-border-secondary)", padding: "18px" }}>
          <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 14, display: "flex", justifyContent: "space-between" }}>
            <span>Выручка по месяцам</span>
            <span style={{ fontSize: 12, color: "var(--color-text-tertiary)" }}>тёмн. = маржа</span>
          </div>
          <div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 110 }}>
            {stats.months.map((m, i) => {
              const hRev    = Math.max((m.revenue / maxMonthRev) * 90, m.revenue > 0 ? 6 : 2);
              const hMargin = m.revenue > 0 ? Math.max((m.margin / maxMonthRev) * 90, 2) : 0;
              return (
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                  {m.revenue > 0 && <div style={{ fontSize: 8, color: "var(--color-text-tertiary)" }}>{fmtK(m.revenue)}</div>}
                  <div style={{ width: "100%", position: "relative", height: hRev, background: "#3b62d655", borderRadius: "3px 3px 0 0" }}>
                    <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: Math.min(hMargin, hRev), background: "#3b62d6", borderRadius: "3px 3px 0 0" }} />
                  </div>
              ...
              return (
                <div key={t.id}>
                  ...
                  <div style={{ height: 5, borderRadius: 3, background: "var(--color-background-secondary)", overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${(t.total / maxT) * 100}%`, background: t.color, borderRadius: 3, transition: "width 0.5s" }} />
                  </div>
                </div>
              );
            })}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        {/* Top products */}
        <div style={{ background: "var(--color-background-primary)", borderRadius: "var(--border-radius-lg)", border: "0.5px solid var(--color-border-secondary)", padding: "18px" }}>
          <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 14 }}>Топ позиций по выручке</div>
                  ...
                  <div key={p.name} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 700, color: "#185FA5", flexShrink: 0, marginLeft: 6 }}>{fmtK(p.revenue)}</span>
                      </div>
                      <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 2 }}>
                        <div style={{ flex: 1, height: 3, borderRadius: 2, background: "var(--color-background-secondary)", overflow: "hidden" }}>
                          <div style={{ height: "100%", width: `${(p.revenue / maxProd) * 100}%`, background: "#3b62d6", borderRadius: 2, opacity: 0.7 }} />
                        </div>
                        <span style={{ fontSize: 10, color: p.margin >= 0 ? "#3B6D11" : "#A32D2D", fontWeight: 500, flexShrink: 0 }}>
                          маржа {fmtK(p.margin)}
                        </span>
                      </div>
                    </div>
        ...
        {/* Top clients */}
        <div style={{ background: "var(--color-background-primary)", borderRadius: "var(--border-radius-lg)", border: "0.5px solid var(--color-border-secondary)", padding: "18px" }}>
          <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 14 }}>Топ клиентов</div>
          {stats.topClients.length === 0
            ? <div style={{ textAlign: "center", padding: "20px 0", color: "var(--color-text-tertiary)", fontSize: 13 }}>Нет данных</div>
            : (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {stats.topClients.map((c, i) => {
                  const [avatarBg, avatarColor] = avatarStyle(c.name);
                  return (
                    <div key={c.name} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 24, height: 24, borderRadius: "50%", background: avatarBg, color: avatarColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 700, flexShrink: 0 }}>{initials(c.name)}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          <span style={{ fontSize: 13, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.name}</span>
                          <span style={{ fontSize: 13, fontWeight: 700, color: "#3B6D11", flexShrink: 0, marginLeft: 6 }}>{fmtK(c.total)}</span>
                        </div>
                        <div style={{ display: "flex", gap: 6, alignItems: "center", marginTop: 2 }}>
                          <div style={{ flex: 1, height: 3, borderRadius: 2, background: "var(--color-background-secondary)", overflow: "hidden" }}>
                            <div style={{ height: "100%", width: `${(c.total / maxClient) * 100}%`, background: "#3B6D11", borderRadius: 2, opacity: 0.6 }} />
                          </div>
                          <span style={{ fontSize: 10, color: "var(--color-text-tertiary)", flexShrink: 0 }}>{c.count} доп.</span>
                        </div>
                      </div>
                    </div>
                  );
...
// MAIN PAGE
// ═══════════════════════════════════════════════════════════════════════════════
const DOC_GRID = "32px 36px 110px 130px 1fr 120px 110px 80px 90px 80px";
...
export default function SalesPage() {
  const { accounts, projects, transactions, loading: storeLoading } = useAppStore();
  ...
  const [activeTab,  setActiveTab]  = useState("documents"); // documents | debt | analytics
  ...
  // ── Firestore: sale_documents ──────────────────────────────────────────────
  useEffect(() => {
    if (!auth.currentUser) return;
    setLoadingDocs(true);
    const unsub = onSnapshot(userCol("sale_documents"), (snap) => {
      setSalesDocs(snap.docs.map((d) => ({ id: d.id, _docId: d.id, ...d.data(), _isoDate: normalizeDate(d.data().date || "") }))
  ...
  // ── Firestore: products (read-only, from products page) ───────────────────
  useEffect(() => {
    if (!auth.currentUser) return;
  ...
  // ── Filtered docs ─────────────────────────────────────────────────────────
  const filtered = useMemo(() => salesDocs.filter((d) => {
    if (filters.type   !== "all" && d.type   !== filters.type)   return false;
    if (filters.status !== "all" && d.status !== filters.status) return false;
    if (filters.client && d.clientName !== filters.client) return false;
    ...
    if (search) {
      ...
      if (!(d.number?.toLowerCase().includes(q) || d.clientName?.toLowerCase().includes(q) || d.notes?.toLowerCase().includes(q))) return false;
    }
    return true;
  }), [salesDocs, filters, search]);
  ...
  // ── KPI ───────────────────────────────────────────────────────────────────
  const kpi = useMemo(() => {
    const revenue = filtered.filter((d) => d.status === "paid").reduce((s, d) => s + (d.total || 0), 0);
    const debt    = filtered.filter((d) => d.status !== "cancelled" && d.status !== "paid")
                            .reduce((s, d) => s + Math.max(0, (d.total || 0) - (d.paid || 0)), 0);
    const overdue = filtered.filter((d) => d.status === "overdue" || (d.dueDate && d.dueDate < todayISO() && d.status !== "paid" && d.status !== "cancelled")).length;
    const margin  = filtered.filter((d) => d.status === "paid").reduce((s, d) => s + ((d.subtotal || 0) - (d.costTotal || 0)), 0);
    return { revenue, debt, overdue, margin, total: filtered.length };
  }, [filtered]);
  ...
  const handleSave = async (form) => {
    try {
      const data = {
        ...
        total:     form.total     || 0,
        costTotal: form.costTotal || 0,
        paid:      parseFloat(form.paid) || 0,
        updatedAt: serverTimestamp(),
      };
      if (form._docId) {
        await updateDoc(userDoc("sale_documents", form._docId), data);
      } else {
        await addDoc(userCol("sale_documents"), { ...data, createdAt: serverTimestamp() });
      }
  ...
  const handleDelete = async (docId) => {
    ...
    try { await deleteDoc(userDoc("sale_documents", docId)); }
    catch (e) { setLocalError("Ошибка: " + e.message); }
    setDocModal(null);
  };
  ...
  const handleBulkStatus = async (statusId) => {
    for (const id of [...selected]) {
      ...
      try { await updateDoc(userDoc("sale_documents", d._docId), { status: statusId, updatedAt: serverTimestamp() }); } catch {}
    }
    setSelected(new Set());
  };
  ...
  const handleBulkDelete = async () => {
    ...
    for (const id of [...selected]) {
      const d = salesDocs.find((x) => x.id === id);
      if (!d?._docId) continue;
      try { await deleteDoc(userDoc("sale_documents", d._docId)); } catch {}
    }
    setSelected(new Set());
  };

  const loading = storeLoading || loadingDocs;
  const TABS = [["documents", "Документы"], ["debt", "Дебиторка"], ["analytics", "Аналитика"]];
    ...
    <div style={{ fontFamily: "var(--font-sans)", height: "100%", overflow: "hidden", background: "var(--color-background-tertiary)", color: "var(--color-text-primary)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flexShrink: 0, zIndex: 200, background: "var(--color-background-primary)" }}>
        <div style={{ borderBottom: "0.5px solid var(--color-border-tertiary)", padding: "16px 24px 0" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <h1 style={{ fontSize: 20, fontWeight: 500, margin: 0 }}>Продажи</h1>
        ...
        <div style={{ padding: "10px 24px", display: "flex", gap: 8, borderBottom: "0.5px solid var(--color-border-tertiary)", background: "var(--color-background-primary)" }}>
          {[
            { label: "Оплачено (выручка)",   value: kpi.revenue,  color: "#3B6D11",                             plain: false },
            { label: "Дебиторка (долг)",      value: kpi.debt,     color: kpi.debt > 0 ? "#A32D2D" : "#3B6D11",  plain: false },
            { label: "Маржа",                 value: kpi.margin,   color: kpi.margin >= 0 ? "#3B6D11" : "#A32D2D",plain: false },
            { label: "Просроченных",          value: kpi.overdue,  color: kpi.overdue > 0 ? "#A32D2D" : "var(--color-text-tertiary)", plain: true },
            { label: "Документов в выборке",  value: kpi.total,    color: "var(--color-text-primary)",            plain: true },
          ].map(({ label, value, color, plain }) => (
            <div key={label} style={{ flex: 1, background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "8px 14px" }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 600 }}>
                {plain ? <span style={{ color }}>{value}</span> : <AnimNum value={value} color={color} />}
              </div>
            </div>
          ))}
        </div>
        ...
        {activeTab === "documents" && !loading && (
          <div style={{ background: "var(--color-background-primary)", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
            <div style={{ minWidth: 980, display: "grid", gridTemplateColumns: DOC_GRID, padding: "0 16px" }}>
              <div style={{ ...TH, display: "flex", alignItems: "center" }}>
                <input type="checkbox" checked={allSelected} onChange={toggleAll} style={{ cursor: "pointer" }} />
              </div>
              <div style={TH} />
              {["Дата", "Номер", "Клиент", "Тип", "Статус", "Итого", "Долг", "Срок"].map((h, i) => (
                <div key={h} style={{ ...TH, textAlign: i >= 5 ? "right" : "left" }}>{h}</div>
              ))}
            </div>
          </div>
        )}
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column", background: "var(--color-background-primary)" }}>
            ...
            <div style={{ minWidth: 980 }}>
              {grouped.length === 0 && (
                <div style={{ padding: "60px 24px", textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 15, color: "var(--color-text-tertiary)", marginBottom: 8 }}>
                    {salesDocs.length === 0 ? "Нет документов. Нажмите + Документ или клавишу N" : "Ничего не найдено по фильтрам"}
                  </div>
                  {(hasFilters || search) && (
              ...
              {grouped.map(([date, rows]) => (
                <div key={date}>
                  ...
                  {rows.map((d) => {
                    const [avatarBg, avatarColor] = avatarStyle(d.clientName || "?");
                    ...
                    return (
                      <div key={d.id}
                        style={{ display: "grid", gridTemplateColumns: DOC_GRID, padding: "0 16px", borderBottom: "0.5px solid var(--color-border-tertiary)", background: isChecked ? "var(--color-background-info)" : "transparent", alignItems: "center", position: "relative", transition: "background 0.1s, padding-right 0.15s", borderLeft: isOverdue ? "2px solid #A32D2D" : "2px solid transparent" }}
                      ...
                      >
                        ...
                        <div style={{ padding: "10px 0" }}>
                          <div style={{ width: 26, height: 26, borderRadius: "50%", background: avatarBg, color: avatarColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 600 }}>{initials(d.clientName)}</div>
                        </div>
                        ...
                        <div style={{ padding: "10px 8px", cursor: "pointer", overflow: "hidden" }} onClick={() => setDocModal({ doc: d })}>
                          <div style={{ fontSize: 13, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.clientName || "—"}</div>
                        ...
                        {/* Total */}
                        <div style={{ padding: "10px 8px", textAlign: "right", fontSize: 14, fontWeight: 700, color: "#185FA5", whiteSpace: "nowrap", cursor: "pointer" }} onClick={() => setDocModal({ doc: d })}>
                          {fmt(d.total || 0)}
                        </div>
                            ...
                            ? <>
                                ...
                                {dl !== null && (
                                  <div style={{ fontSize: 9, color: isOverdue ? "#A32D2D" : "var(--color-text-tertiary)" }}>
                                    {isOverdue ? `−${Math.abs(dl)} дн.` : `+${dl} дн.`}
                                  </div>
                                )}
                              </>
              ...
              {filtered.length > 0 && (
                <div style={{ padding: "12px 24px", borderTop: "0.5px solid var(--color-border-tertiary)", display: "flex", gap: 24, flexWrap: "wrap", fontSize: 13, color: "var(--color-text-secondary)" }}>
                  ...
                  <span>Итого: <strong style={{ color: "#185FA5" }}>{fmt(filtered.reduce((s, d) => s + (d.total || 0), 0))}</strong></span>
                  ...
                  <span style={{ marginLeft: "auto", color: "var(--color-text-tertiary)" }}>Маржа: <strong style={{ color: kpi.margin >= 0 ? "#3B6D11" : "#A32D2D" }}>{fmt(kpi.margin)}</strong></span>
                </div>
              )}
            </div>
          </div>
        ...
        {!loading && activeTab === "debt" && (
          <DebtTab documents={salesDocs} onOpenDoc={(d) => setDocModal({ doc: d })} />
        )}

        {/* ── ANALYTICS TAB ── */}
        {!loading && activeTab === "analytics" && (
          <AnalyticsTab documents={salesDocs} />
        )}
      </div>
    </div>
  );
